    ...
    def _compute_best_work_time(self):
        for record in self:
            ...
            for shift in weekday_attendances:
                _logger.debug(f"Evaluating shift {shift.name}, hour_from: {shift.hour_from}, hour_to: {shift.hour_to}")
                ...
                if record.check_in_computed:
                    ...
                    _logger.debug(f"Time difference to shift start: {time_diff_start}, to shift end: {time_diff_end}")
                    min_diff = min(time_diff_start, time_diff_end)
                    _logger.debug(f"Minimum time difference: {min_diff}")
                    if min_diff < lowest_time_diff:
                        _logger.debug(f"New closest shift found: {shift.name} with time difference {min_diff}")
                        lowest_time_diff = min_diff
                        closest_shift = shift.id
                    if min_diff == time_diff_start:
                        _logger.debug(f"Closest to shift start for shift {shift.name}")
                        closest_to = 'start'
                    else:
                        _logger.debug(f"Closest to shift end for shift {shift.name}")
                        closest_to = 'end'
